...
import net.nashat.Bandwidth
import net.nashat.cartesianProduct
...
class IdealPubsub2(
    ...
    companion object {
        ...
        fun calculateRelativeTime(nodeCount: Int, msgPartCount: Int, latencyRounds: Int): Double {
            val messageTransmitTime = 1.seconds
            val bandwidth = 10.mbitsPerSecond
            val messageSize = bandwidth.getTransmitSize(messageTransmitTime)
            val roundTime = messageTransmitTime / msgPartCount
            val idealPubsub2 = IdealPubsub2(
                SimParams(
                    nodeCount,
                    messageSize.toLong(),
                    msgPartCount,
                    bandwidth,
                    roundTime * latencyRounds
                )
            )
            idealPubsub2.simulate()
            return idealPubsub2.curRound.toDouble() / msgPartCount
        }
    }
}